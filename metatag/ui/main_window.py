...
class MainWindow(QMainWindow):
    ...
    def _on_row_changed(self, visual_row: int) -> None:
        ...
        if visual_row < 0:
            return
        self._current_index = self._proxy_model.mapToSource(self._file_list.model().index(visual_row, 0)).row()
    ...
    def _open_files(self, file_paths: "list[str] | None" = None) -> None:
        ...
        self._tracks.clear()
        self._track_model.beginResetModel()
        self._tracks.extend(Track(f) for f in files)
        self._track_model.endResetModel()
    ...
    def _set_cover_from_file(self, path: str) -> None:
        try:
            ...
            if self._apply_to_selected_check.isChecked():
                for row in {i.row() for i in self._file_list.selectedIndexes()}:
                    if 0 <= row < len(self._tracks):
                        self._tracks[row].cover_art = image
            elif 0 <= self._current_index < len(self._tracks):
                self._tracks[self._current_index].cover_art = image
    ...
    def _batch_rename(self) -> None:
        ...
        from ..utils.rename_utils import rename_tracks

        dialog = RenameDialog(self)
        if not dialog.exec():
            return
        pattern = dialog.pattern()
        results = rename_tracks(self._tracks, pattern)
        success = sum(1 for v in results.values() if v)
        failed  = len(results) - success
        self._track_model.refresh()
        msg = f"Renamed {success} file(s)"
        if failed:
            msg += f", {failed} failed"
        self.statusBar().showMessage(msg, 5000)
        if failed:
            QMessageBox.warning(
                self, "Rename Errors", f"{failed} file(s) could not be renamed."
            )
    ...
    @Slot()
    def _batch_rename(self) -> None:
        """Rename selected tracks based on their metadata."""
        indexes = self._file_list.selectionModel().selectedRows()
        if not indexes:
        ...
        if dialog.exec():
            pattern = dialog.pattern()
            # Rename logic (we'll keep it simple: rename on disk, update the path object)
            success = 0
            for idx in [self._proxy_model.mapToSource(i).row() for i in indexes]:
                track = self._tracks[idx]
                tags = {
                    "artist": track.artist, "album": track.album, "title": track.title,
                    "track_number": track.track_number, "disc_number": track.disc_number, "year": track.year
                }
