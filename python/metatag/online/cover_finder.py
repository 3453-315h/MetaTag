...
class CoverFinder(QObject):
    ...
    def _on_releases_fetched(self, releases: List[dict]) -> None:
        """Handle fetched releases — each item is a dict with at least an 'id' key."""
        if not releases:
            self.fetch_error.emit("No releases found for artist/album")
            return
        # MusicBrainzLookup emits list-of-dicts; extract the release ID from the first
        release_id = releases[0].get("id", "") if isinstance(releases[0], dict) else str(releases[0])
        if not release_id:
            self.fetch_error.emit("No valid release ID in MusicBrainz response")
            return
