...
def test_main_window_apply_to_selected(qtbot):
    ...
    window._add_files([str(t.file_path) for t in mock_tracks])
    window._tracks = mock_tracks

    # Initialize the track model properly
...
def test_main_window_cover_art_drag_drop(qtbot):
    ...
    mock_track = _make_mock_track()
    window._add_files([str(mock_track.file_path)])
    window._tracks = [mock_track]
    window._current_index = 0
    window._track_model.beginResetModel()
    window._track_model.endResetModel()
...
def test_main_window_navigation(qtbot):
    ...
    with patch("metatag.ui.main_window.Track") as MockTrack:
        ...
        window._add_files([f"track{i}.mp3" for i in range(3)])
...
def test_main_window_nav_label(qtbot):
    ...
    with patch("metatag.ui.main_window.Track") as MockTrack:
        MockTrack.side_effect = tracks
        window._add_files([f"track{i}.mp3" for i in range(5)])
