...
def test_main_window_apply_to_selected(qtbot):
    ...
    # Populate file-list table via the model
...
def test_main_window_cover_art_drag_drop(qtbot):
    ...
    window._tracks = [mock_track]
    window._track_model.beginResetModel()
    window._track_model._tracks = [mock_track]
    window._track_model.endResetModel()
    window._current_index = 0
...
def test_main_window_menu_actions(qtbot):
    ...
    assert any("CSV Report…" in t for a in file_menu.actions() if a.menu() for t in [sa.text() for sa in a.menu().actions()])
