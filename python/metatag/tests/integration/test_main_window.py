...
def test_main_window_menu_actions(qtbot):
    ...
    assert any("CSV Report" in t for a in file_menu.actions() for t in [a.text()] if a.menu() is None) or any("CSV Report" in t for a in file_menu.actions() if a.menu() is not None for sa in a.menu().actions() for t in [sa.text()])
    ...
    assert any("Find & Replace" in t for t in edit_texts)
