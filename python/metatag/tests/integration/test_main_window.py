...
def test_main_window_apply_to_selected(qtbot):
    ...
    with patch("metatag.ui.main_window.QFileDialog.getOpenFileNames", return_value=(["fake1.mp3", "fake2.mp3", "fake3.mp3"], "")):
        with patch("metatag.ui.main_window.Track") as MockTrack:
            MockTrack.side_effect = mock_tracks
            window._open_files()
...
def test_main_window_cover_art_drag_drop(qtbot):
    ...
    with patch("metatag.ui.main_window.QFileDialog.getOpenFileNames", return_value=(["fake.mp3"], "")):
        with patch("metatag.ui.main_window.Track") as MockTrack:
...
def test_main_window_nav_label(qtbot):
    ...
    with patch("metatag.ui.main_window.QFileDialog.getOpenFileNames", return_value=([f"fake{i}.mp3" for i in range(5)], "")):
        with patch("metatag.ui.main_window.Track") as MockTrack:
            MockTrack.side_effect = tracks
            window._open_files()
